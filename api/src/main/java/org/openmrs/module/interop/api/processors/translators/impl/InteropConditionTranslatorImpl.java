...
public class InteropConditionTranslatorImpl implements InteropConditionTranslator {
    ...
    public org.hl7.fhir.r4.model.Condition toFhirResource(@Nonnull Condition condition) {
        ...
        if (condition.getVerificationStatus() != null) {
            fhirCondition.setVerificationStatus(new CodeableConcept().addCoding(new Coding(
                    "http://hl7.org/fhir/ValueSet/condition-ver-status",
                    condition.getVerificationStatus().toString().toLowerCase(), condition.getVerificationStatus().toString())));
        }
